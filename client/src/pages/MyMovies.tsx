...
import { HiArrowLeft, HiFilm, HiBookmark, HiEye, HiCheck, HiPlusCircle, HiTrash, HiXMark, HiStar, HiQueueList } from 'react-icons/hi2';
import { collectionApi, type MarkStatus, type UserMark, type UserList } from '@/services/collectionApi';

const statusTabs = [
  { key: undefined as MarkStatus | undefined, label: '全部', icon: HiFilm },
  { key: 'wish' as MarkStatus, label: '想看', icon: HiBookmark },
  { key: 'doing' as MarkStatus, label: '在看', icon: HiEye },
  { key: 'done' as MarkStatus, label: '看过', icon: HiCheck },
];
...
export default function MyMovies() {
    ...
    try {
      if (tab === 'marks') {
        const [m, s] = await Promise.all([
          collectionApi.getMarks('movie', statusFilter),
          collectionApi.getMarkStats('movie'),
        ]);
        setMarks(m);
        setStats(s);
      } else {
        setLists(await collectionApi.getLists('movie'));
      }
    } catch {
      setError('我的影视数据加载失败，请确认登录状态和后端服务后重试。');
    }
  ...
  const handleCreateList = async () => {
    ...
    await collectionApi.createList('movie', newName.trim());
    setNewName('');
    setShowCreate(false);
    fetchData();
  };
  ...
  return (
    <div className="min-h-screen pt-24 pb-32 px-4 sm:px-6 max-w-5xl mx-auto">
      <div className="flex items-center gap-3 mb-2">
        <Link to="/movies" className="p-2 rounded-xl hover:bg-white/10 transition-colors"><HiArrowLeft className="w-5 h-5 text-gray-400" /></Link>
        <HiFilm className="w-6 h-6 text-violet-400" />
        <h1 className="text-3xl font-bold"><span className="gradient-text">我的影视</span></h1>
      </div>

      <div className="grid grid-cols-3 gap-3 my-6 ml-12">
        {[
          { label: '想看', count: stats.wish, color: 'from-blue-500 to-cyan-400' },
          { label: '在看', count: stats.doing, color: 'from-yellow-500 to-orange-400' },
          { label: '看过', count: stats.done, color: 'from-green-500 to-emerald-400' },
        ].map((s) => (
      ...
      <div className="flex gap-2 mb-6">
        ...
        <button onClick={() => setTab('lists')} className={`px-5 py-2.5 rounded-xl text-sm font-medium transition-all ${tab === 'lists' ? 'bg-white/10 text-white border border-white/10' : 'text-gray-400 hover:text-white'}`}>
          <HiQueueList className="w-4 h-4 inline mr-1" /> 片单
        </button>
      </div>
      ...
      {tab === 'marks' && (
        <>
          ...
          {loading ? (
            ...
            <div className="space-y-3">
              {marks.map((m, i) => (
                <motion.div key={m.itemId} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.05 }}>
                  <Link to={`/movies/${m.itemId}`} className="block glass rounded-xl p-4 hover:bg-white/5 transition-colors group">
                      ...
                      <div>
                        ...
                        <div className="flex items-center gap-3 mt-1">
                          <span className={`text-xs px-2 py-0.5 rounded-full ${m.status === 'wish' ? 'bg-blue-500/10 text-blue-400' : m.status === 'doing' ? 'bg-yellow-500/10 text-yellow-400' : 'bg-green-500/10 text-green-400'}`}>
                            {m.status === 'wish' ? '想看' : m.status === 'doing' ? '在看' : '看过'}
                          </span>
            ...
            <div className="text-center py-16 glass rounded-2xl"><p className="text-4xl mb-3">🎬</p><p className="text-gray-400">暂无标记的影视</p></div>
          )}
        </>
      )}
      ...
      {tab === 'lists' && (
        <>
          <button onClick={() => setShowCreate(true)}
            ...
            <HiPlusCircle className="w-5 h-5" /> 新建片单
          </button>
          ...
          {loading ? (
            ...
            <div className="space-y-3">
              {lists.map((l, i) => (
                <Link key={l.id} to={`/movies/list/${l.id}`}>
                  <motion.div initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.05 }}
                    ...
                    <div className="w-14 h-14 rounded-lg bg-gradient-to-br from-violet-600 to-purple-600 flex items-center justify-center flex-shrink-0 group-hover:shadow-neon-purple transition-shadow">
                      <HiQueueList className="w-6 h-6 text-white" />
                    </div>
                    <div className="flex-1">
                      <h3 className="text-white font-medium group-hover:text-violet-300 transition-colors">{l.name}</h3>
                      <p className="text-xs text-gray-500">{l.items.length} 部 · {new Date(l.createdAt).toLocaleDateString('zh-CN')}</p>
                    </div>
            ...
            <div className="text-center py-16 glass rounded-2xl"><p className="text-4xl mb-3">📝</p><p className="text-gray-400">还没有片单</p></div>
          )}
        </>
      )}
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 bg-black/60 flex items-center justify-center p-4">
            <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} exit={{ scale: 0.9 }} className="glass rounded-2xl p-6 w-full max-w-md border border-white/10">
              <div className="flex justify-between mb-4">
                <h3 className="text-lg font-bold text-white">新建片单</h3>
              ...
              <input value={newName} onChange={(e) => setNewName(e.target.value)} placeholder="片单名称"
